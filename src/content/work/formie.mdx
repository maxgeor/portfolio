...
<Section title="Only supporting one-way actions">
  ...
  >
    ...
    - `Two-way`: An action that _returns data_<br/>(ex: Seeing an account's balance, asking for an NFT's characteristics) 
    
    Two-way actions are complex. Different data needs different layouts, with non-blockchain data getting in the mix. These constraints make a custom UI necessary to deliver a great experience.

    One-way actions are simple — they need some fields, a button, and a straightforward layout. Our research also <a class="anchor" href="#research">found that they're underserved</a> relative to other tooling.

    We decided to focus on one-way actions, shrinking our scope.
...
<Section title="Using human words">
  ...
  >
    ...
    We realized that static language couldn't convey the specific context of every action. To make forms flexible, we let people add a title and description. Now, people could share specific details and instructions for their form.
...
<Section title="Tradeoffs">
  <SubSection>
    ...
    By <a class="anchor" href="https://basecamp.com/shapeup/3.5-chapter-14#compare-to-baseline" target="_blank">comparing to our baseline</a>, we cut non-essential features:
  </SubSection>
  ...
  >
    ...
    I wanted every part of a form to be editable:
