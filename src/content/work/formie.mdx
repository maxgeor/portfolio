...
<Section title="Research">
  <Section title="Most actions just need a form" isSubsection={true}>
    The first thing we noticed was how Etherscan's "frontend" was a list of basic forms.

    While it worked for projects, we weren't sold on the best layout being a plain old form... even though it turned out to be.

    <FootnoteParagraph>
      We learned how contract actions are radically simple<sup>1</sup>, with most actions only take 1-2 inputs and return little to no data. The straightforward, input/output nature of most action justified form-based layouts as an approrpiate pattern, not an arbitrary decision.
      <Footnote number={1}>
...
<Section title="Idea">
  {/* How did you come to the idea for the form builder?
  - Seeing all the actions as mini forms, we wondered realized how much better we could make them by applying patterns from form builders like Google/Airtable Forms:
    - Give every action their own page
    - Add clear, helpful copy
    - Make it easy to customize
    - Thoughtful error handling
    - Give feedback about the transaction

  - This worked becasue one-way actions lent themselves to form-like layouts */}
