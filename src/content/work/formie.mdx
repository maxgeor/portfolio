...
import Details from "../../components/prose/Details.astro";
import Section from "../../components/prose/Section.astro";
import SubSection from "../../components/prose/SubSection.astro";
import Tooltip from "../../components/prose/Tooltip.astro";
import Paragraph from "../../components/prose/Paragraph.astro";
...
<Section title="Problem">
  ...
  >
    <mark>Etherscan frontends suck</mark>.
    ...
    <Details summary="Etherscan? Smart contracts?">
      ...
      Instead of making their own frontend, blockchain devs use “frontend generators”. These spit out a basic UI based on your code, saving tons of time.

      Etherscan is the most popular frontend generator, used by multiple Ethereum projects.

      The problem?
    </Details>

    {/* <details>
      <summary>Etherscan? Smart contracts?</summary>
      <div>
        This is how developers make blockchain apps:
        ...
        Instead of making your own frontend, blockchain devs use “frontend generators”. These create basic UI based on your code, saving you time & effort.

        Etherscan is the most popular frontend generator, used by tons of Ethereum projects.
  ...
  <SubSection
    images={[
      { 
        ...
        alt: "First sketch for form builder UI",
        isPadded: true
      }
    ]}
  >
    Our big breakthrough was realizing that smart contract actions are mini forms. 
    
    They have a couple fields and a button that does something when pressed. Framing smart contract actions as forms made Etherscan's shortcomings clear:

    - There's no confirmation if anything worked
    - You can't link someone to the action they want
    - There's no copy guiding people or explaining things

    With this, we got our <mark>high-level solution: A form builder, like Google Forms, but for smart contracts</mark>.
...
<Section title="Research">
  <SubSection>
    ...
    - People used Etherscan as a "good enough" interface. Teams used it for admin-like actions that didn't deserve fancy interfaces. Blockchain developers, liked working in the backend, delegating the frontend to Etherscan.
    - Everyone who used Etherscan knew it sucked (visually & functionally) but weren't willing to build something themselves.
    {/* Takeaway: We should focus on serving simple actions that don't need complex interfaces. Takeaway: The bar was _really_ low.  We had room to cut lots of functionality. */}
...
<Section title="Challenges">
  <SubSection>
    ...
    We needed a way to deter scammers without making the product harder to build or use.

    ## Simple vs Secure
    Simple vs Secure Simple vs Secure Simple vs Secure Simple vs Secure vs Simple vs Secure Simple vs Secure Simple vs Secure

    ## Flexibility
    Not only does the form need to work with most smart contracts, but it also needs to work with most brands. The product needed an unopinioned aestheic.

    Why does it need an universal aesthetic? Well this page will basically be an extension of a project, acting as another page in their product's website, or it'll be the only interface to a product.

    Becuase it needs to work in a bunch of different contents, it doesn't call for a distinctive or particulartiy noteworthy brand. In a way, the "brand" or Formie should be invisible to anyone using a form.

    Formie's aestheic should be the opposite of distinctive. To users, there should be no brand.
