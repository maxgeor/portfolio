...
<Section title="Research">
  <Section title="Most actions just need a form" isSubsection={true}>
    The first thing we noticed was how Etherscan's "frontend" was a list of basic forms. Despite our skeptism, a form-based layout turned out to be an appropriate pattern for smart contract actions.

    <FootnoteParagraph>
      We learned how contract actions are radically simple<sup>1</sup>, with most actions only take 1-2 inputs and return little to no data. Having few steps and elements makes a focused, linear flow the perfect structure to help people call contracts.
      <Footnote number={1}>
...
<Section title="Idea">
  Realizing that smart contracts worked like mini forms gave us an idea: **What if we took common patterns from form builders and applied them to smart contracts?** 
  
  We could make huge UX improvements by...

    - [Giving every action their own page](#one-page-one-action)
...
<Section title="Key Decisions">
  ...
  <Section title="Only supporting one-way actions" isSubsection={true}>
    ...
    Becasue we couldn't create a uniquely excellent interface for all two-way methods, we filtered them out from searches to only support one-way actions.

    Ditching two-ways axed our scope, making the product easier to build and use.
