...
Formie got featured by the Milk Road and was picked as a top project from their Milk Money Cohort. 

While it was used by teams and developers, we put it on auto-pilot. It worked great for basic uses, but we discovered a lot more cases where only a _little_ customization was needed. The tweaks were small, but different across a bunch of users. There was no easy way to elegantly support these customizations in a standardized way.

Since the improvements didn't make sense to invest time and money into, we decided to keep Formie as is and just support the simple usecases.
