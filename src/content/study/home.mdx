...
<Section id="1.00" title='Problem' hasBorder={false}>
  <p slot="context">Wavy’s homepage made it **hard to find the events you care about**. It was a single calendar view with cells showing *events*, *notable days* and *suggested events*. These **items were undifferentiated**. At a glance, you couldn’t tell them apart. Events items themselves had no *status*, so you couldn’t tell which ones you’ve been invited to or already responded to. The month view made it **difficult to see activity beyond the current month**. You’d forget all about events that 1-2 months away and go on scavenger hunts to find that event from 3 months ago.</p>
  {/* <Carousel
    slides={[
      {
        id: "1.01",
